...
PionAnglaise::PionAnglaise(int x, int y , std::string c):PieceDame(x,y,c),role("P"){}

std::string PionAnglaise::getRole() const{
  return role;
}

void PionAnglaise::promotion(std::vector<Piece*> & joueur_courant, int taille){
    int x1 ;
    int y1;
  if(this->couleur == "blanc" && this->y == taille -1){
    ...
    y1 = this->getY();
    this->setCoord(11,11);
    Piece* dame = new DameAnglaise(x1,y1,"blanc");
    ...
    if(this->couleur == "noir" && this->y == 0){
      ...
      y1 = this->getY();
      this->setCoord(11,11);
      Piece* dame = new DameAnglaise(x1,y1,"noir");
      joueur_courant.push_back(dame);
    }
  }
}

int PionAnglaise::valeurDePrise(std::vector<Piece*> & joueur_courant, std::vector<Piece*> & joueur_adverse, std::vector<Piece*> & cimtierre,int taille,int x, int y){
  int hd = 0;
  int hg = 0;
  int bd = 0;
  int bg = 0;
 ...
 std::vector<std::pair<int, int>> PionAnglaise::mouvementsPossible(int taille, std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse){
...
std::vector<std::pair<int,int>> PionAnglaise::listeMangeable(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse, std::vector<Piece*> & bientot_mort,int taille,int x, int y){
...
std::pair<int, int> PionAnglaise::nextMove(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse,int taille,int x2, int y2){
...
//propose les Pieces mangeable -> choix -> jump -> propose choix tant que les jumps ne sont pas fini
 void PionAnglaise::jumpRafle(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse, int taille,int x_initial, int y_initial , int rafle_max){
...
void PionAnglaise::jumpRafleBot(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse, int taille,int x_initial, int y_initial , int rafle_max){
