...
DameAnglaise::DameAnglaise(int x, int y , std::string c):PieceDame(x,y,c),role("D"){}

std::string DameAnglaise::getRole() const{
  return role;
}

void DameAnglaise::promotion(std::vector<Piece*> & joueur_courant , int taille) {}

int DameAnglaise::valeurDePrise(std::vector<Piece*> & joueur_courant, std::vector<Piece*> & joueur_adverse, std::vector<Piece*> & cimtierre,int taille,int x, int y){
  int hd = 0;
  int hg = 0;
  int bd = 0;
  int bg = 0;
 ...
 std::vector<std::pair<int, int>> DameAnglaise::mouvementsPossible(int taille, std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse){
    ...
    if(this->x == taille - 1){
      ...
      if(this->y == 0){
        ...
        if(this->y == taille - 1){
          ...
          d = std::make_pair(x+1,y-1);
          ListeCoordonnees.push_back(d);
          u = std::make_pair(x-1,y+1);
          ListeCoordonnees.push_back(u);
          v = std::make_pair(x-1,y-1);
          ListeCoordonnees.push_back(v);
        }
      }
    }
  }
...
//renvoi liste de position que le Piece peut manger
 std::vector<std::pair<int,int>> DameAnglaise::listeMangeable(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse, std::vector<Piece*> & bientot_mort,int taille,int x, int y){
...
std::pair<int, int> DameAnglaise::nextMove(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse,int taille,int x2, int y2){
...
//propose les Pieces mangeable -> choix -> jump -> propose choix tant que les jumps ne sont pas fini
 void DameAnglaise::jumpRafle(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse, int taille,int x_initial, int y_initial , int rafle_max){
...
void DameAnglaise::jumpRafleBot(std::vector<Piece*> joueur_courant, std::vector<Piece*> joueur_adverse, int taille,int x_initial, int y_initial , int rafle_max){
